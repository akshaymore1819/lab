...
const Login = () => {
  const [formData, setFormData] = useState({ name: '', mobile: '', email: '', otp: '' });
  const [loading, setLoading] = useState({ otp: false, submit: false });
  const [errors, setErrors] = useState({ name: '', mobile: '', email: '', otp: '' });
  ...
  const handleSendOtp = () => {
    ...
    setTimeout(() => {
      setOtpSent(true);
      setAlert({ show: true, message: `OTP sent to ${formData.email}`, severity: 'success' });
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setAlert({ show: true, message: `Registration successful, ${formData.name}!`, severity: 'success' });
  ...
  return (
    <Box className="rl-login-root">
      <Box className="rl-login-card">
        <Typography variant="h4" className="rl-login-title">User Registration</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField
            fullWidth label="Full Name" name="name" value={formData.name} onChange={handleChange}
            error={!!errors.name} helperText={errors.name}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start"><Person color="primary" /></InputAdornment>
              )
            }}
          />
          ...
          <TextField
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start"><Phone color="primary" /></InputAdornment>
              )
            }}
          />
          ...
          <TextField
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start"><Email color="primary" /></InputAdornment>
              ),
              endAdornment: (
                <InputAdornment position="end">
                  <Button
                    onClick={handleSendOtp} disabled={otpSent || loading.otp}
                    variant="outlined" size="small" className="rl-login-otp-btn"
                  >
        ...
        <Box className="rl-login-help">
          <Typography variant="body2">Need help?</Typography>
          <IconButton onClick={handleWhatsApp} className="rl-login-whatsapp">
